	...
	*/
	...
	public MainCanvas(){
		...
		for(int i =0;i <heroLeftImg.length;i++){
			heroLeftImg[i] = Image.createImage("sayo" + "i" + "2");	//后缀为2，向左
		}
		for(int i =0;i <heroRightImg.length;i++){
			heroRightImg[i] = Image.createImage("sayo" + "i" + "6");	//后缀为6，向右
		}
		for(int i =0;i <heroUpImg.length;i++){
			heroUpImg[i] = Image.createImage("sayo" + "i" + "4");	//后缀为4，向上
		}
		for(int i =0;i <heroDownImg.length;i++){
			heroDownImg[i] = Image.createImage("sayo" + "i" + "0");	//后缀为0，向下
		}
	...
	public void getImage(int action,Image heroMoveImg_1,Image hereMoveImg_2){
		try{
			if(isAction){
				currentImg = Image.createImage(heroMoveImg_1);
				isAction = !isAction;
			}else if(!isAction){
				currentImg = Image.createImage(hereMoveImg_2);
				isAction = !isAction;
			}
		}catch(IOException e){
			e.printStackTrace();
